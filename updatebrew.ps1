...
$formulaPath = "$PSScriptRoot/Formula/rclone-encrypt-test-glm.rb"
...
foreach ($platform in $platforms) {
    $file = Join-Path $artifactsDir "rclone-encrypt-test-glm-$platform.tar.gz"
    if (-not (Test-Path $file)) {
        throw "Unable to locate rclone-encrypt-test-glm-$platform.tar.gz at $file"
    }
...
class RcloneEncryptTestGlm < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "$base/rclone-encrypt-test-glm-darwin-arm64.tar.gz"
      sha256 "$($hash['darwin-arm64'])"
    else
      url "$base/rclone-encrypt-test-glm-darwin-amd64.tar.gz"
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "$base/rclone-encrypt-test-glm-linux-arm64.tar.gz"
      sha256 "$($hash['linux-arm64'])"
    else
      url "$base/rclone-encrypt-test-glm-linux-amd64.tar.gz"
